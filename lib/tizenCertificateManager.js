...
const caPriKeyPath = path.resolve(utils.extensionRoot, 'tools', 'certificate-generator', 'certificates', 'developer', 'tizen-developer-ca-privatekey.pem');
const caCertPath = path.resolve(utils.extensionRoot, 'tools', 'certificate-generator', 'certificates', 'developer', 'tizen-developer-ca.cer');
const distributorPublicCA = path.resolve(utils.extensionRoot, 'tools', 'certificate-generator', 'certificates', 'distributor', 'sdk-public', 'tizen-distributor-ca.cer');
const distributorPublicSigner = path.resolve(utils.extensionRoot, 'tools', 'certificate-generator', 'certificates', 'distributor', 'sdk-public', 'tizen-distributor-signer.p12');
const distributorPartnerCA = path.resolve(utils.extensionRoot, 'tools', 'certificate-generator', 'certificates', 'distributor', 'sdk-partner', 'tizen-distributor-ca.cer');
const distributorPartnerSigner = path.resolve(utils.extensionRoot, 'tools', 'certificate-generator', 'certificates', 'distributor', 'sdk-partner', 'tizen-distributor-signer.p12');
...
module.exports = class TizenCM {
    ...
    async init() {
        ...
        if (!fs.existsSync(caPriKeyPath) || !fs.existsSync(caCertPath)) {
            ...
            let pkgInfo = await downloadMgr.getPackageInfo(utils.tizenDownloadUrl, 'certificate-generator');
            console.info(pkgInfo);
            await downloadMgr.downloadPkg(pkgInfo);
            await downloadMgr.unzipPkgDir(utils.tmpDir, utils.extensionRoot, 'data');
            fs.rmdirSync(utils.tmpDir);
        }
    }
