...
function createProfilesXml(profilePath) {
...
function readProfilesData(profilePath) {
    if (!fs.existsSync(profilePath)) {
        createProfilesXml(profilePath);
    }

    return fs.readFileSync(profilePath, {encoding: 'utf-8'});
}

function getProfileJson(profilePath) {
    let profilesData = readProfilesData(profilePath);
...
function syncProfileToFile(profilePath, profiles) {
...
function createProfile(profilePath, profileName, author, distributor1, distributor2, isActive) {
    let newProfile = makeNewProfile(profileName, author, distributor1, distributor2);
    let profileJson = getProfileJson(profilePath);
    ...
    syncProfileToFile(profilePath, profileJson);
}

function removeProfile(profilePath, profileName) {
    let profileJson = getProfileJson(profilePath);
...
function modifyProfile(profilePath, profileName, itemType, profileitem) {
...
function isProfileExist(profilePath, profileName) {
    let profileJson = getProfileJson(profilePath);
...
function activateProfile(profilePath, profileName) {
    let profileJson = getProfileJson(profilePath);
...
function setEmptyActivateProfile(profilePath) {
    let profileJson = getProfileJson(profilePath);
    profileJson.profiles.$.active = '';
    syncProfileToFile(profilePath, profileJson);
}

function listProfile(profilePath) {
    let profileJson = getProfileJson(profilePath);
...
function getProfileKeys(profilePath, profileName) {
    let profileJson = getProfileJson(profilePath);
...
function getProfileItems(profilePath, profileName) {
    let profileJson = getProfileJson(profilePath);
...
function getProfile(profilePath, profileName) {
    let profileJson = getProfileJson(profilePath);
    if (profileJson.profiles.profile == undefined) {
        return null;
    } else {
...
function getActiveProfile(profilePath) {
    let profileJson = getProfileJson(profilePath);
