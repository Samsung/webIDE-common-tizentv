...
class AppLauncher {
  ...
  async checkEmulator() {
    ...
    return this._execAsync(
      `${sdbExec} --emulator devices`,
      function (stdout, resolve, reject) {
        if (stdout.includes(emulatorDev)) {
          resolve();
        } else {
          console.error(
            '[webide-common-tizentv]AppLauncher.checkEmulator:  No running emulator device.'
          );
  ...
  checkChrome(chromeExecPath) {
    ...
    console.error(
      '[webide-common-tizentv]AppLauncher.checkChrome: Chrome executable file is not configured.'
    );
    return Promise.reject(
      `EnvError: Chrome executable file is not configured.`
    );
  }
  ...
  async pushWgt() {
    ...
    if (!existsSync(this.wgtFile)) {
      console.error(
        '[webide-common-tizentv]AppLauncher.pushWgt: ' +
          this.wgtFile +
          ' is not exist'
      );
    ...
    return this._execAsync(
      `${sdbExec} -s ${this.device} push ${this.wgtFile} ${sdkToolTmp}`
    );
  }
  ...
  async installApp() {
    let sdbExec = await tools.getSdbPath();
    return this._execAsync(
      `${sdbExec} -s ${this.device} shell 0 vd_appinstall  ${this.pkgID}.${this.pkgName} ${sdkToolTmp}${this.pkgName}.wgt`
    );
  }
  ...
  async uninstallApp() {
    let sdbExec = await tools.getSdbPath();
    return this._execAsync(
      `${sdbExec} -s ${this.device} shell 0 vd_appuninstall ${this.pkgID}.${this.pkgName}`
    );
  }
  ...
  async executeApp() {
    let sdbExec = await tools.getSdbPath();
    return this._execAsync(
      `${sdbExec} -s ${this.device} shell 0 execute ${this.pkgID}.${this.pkgName}`
    );
  }

  async debugApp(portForward) {
    let sdbExec = await tools.getSdbPath();
    return this._execAsync(
      `${sdbExec} -s ${this.device} shell 0 debug ${this.pkgID}.${this.pkgName}`,
      async (stdout, resolve, reject) => {
        // For debug mode stdout: ... successfully launched pid = xxxx with debug 1 port: xxxxx
        if (
          stdout.includes(`successfully launched`) &&
          ...
          if (portN || portR) {
            this.debugPort = this.debugPort.substring(
              0,
              portN < portR ? portN : portR
            );
          }
          if (portForward) {
            await this.setDebugPort(portForward);
          }
          resolve();
        } else {
          reject(`error: ${stdout}`);
  ...
  async setDebugPort(portForward) {
    let sdbExec = await tools.getSdbPath();
    if (portRegExp.test(this.debugPort)) {
      await this._execAsync(
        `${sdbExec} -s ${this.device} forward tcp:${portForward} tcp:${this.debugPort}`
      );
      this.debugPort = portForward;
      this.isPortForward = true;
      return;
    }
  ...
  openChromeDevTool(chromeExecPath) {
    let ipAddr = this.device;
    if (this.device.includes('emulator') || this.isPortForward) {
      ipAddr = '127.0.0.1';
    }
    let port = this.device.includes('emulator') ? '7011' : this.debugPort;

    const ip = ipAddr.split(':')[0]; //split that case of ip:port

    httpGet(`http://${ip}:${port}/json`, res => {
      let weJson = '';
