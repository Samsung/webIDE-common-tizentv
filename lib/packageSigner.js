...
const ExclusiveCanonicalization =
    require('./exclusive-canonicalization').ExclusiveCanonicalization;
...
class Reference {
    ...
    digest(content) {
        if (this.uri == '#prop') {
            // currently we use const prop digest value, TO BE DONE: this should be calulated
...
class Signature {
    ...
    _listFiles(userExcludeFiles) {
        ...
        if (
            undefined == userExcludeFiles ||
            null == userExcludeFiles ||
            '' == userExcludeFiles
        ) {
            userExcludeFiles = [''];
        } else {
        ...
        function _listDirs(curDir) {
            ...
            dir.forEach(item => {
                if (
                    ...
                    !userExcludeFiles.includes(path.resolve(curDir, item.name))
                ) {
                    if (item.isDirectory()) {
    ...
    _addReferences(fileList) {
        ...
        fileList.forEach(file => {
            let uri = encodeURIComponent(
                file.substring(this.projectRoot.length + 1).replace(/\\/g, '/')
            );
        ...
        propRef.digest(
            this.id == 'AuthorSignature' ? authorPropDgst : distributorPropDgst
        );
    ...
    sign(key, userExcludeFiles) {
        ...
        let canonedXMl = transform.process(node, {
    ...
    generateSignatureXml() {
        return (
            `<Signature xmlns="http://www.w3.org/2000/09/xmldsig#" Id="${this.id}">\n` +
            `${this.signedInfo}` +
            `${this.signatureValue}` +
            `${this.keyInfo}` +
            `<Object Id="prop"><SignatureProperties xmlns:dsp="http://www.w3.org/2009/xmldsig-properties"><SignatureProperty Id="profile" Target="#${
                ...
                this.id == 'AuthorSignature' ? 'author' : 'distributor'
...
module.exports = class PackageSigner {
    ...
    async setProfile(profilePath, profile) {
        let items = profileEditor.getProfileItems(profilePath, profile);
        if (
            items.authorKey == null ||
            items.authorPwd == null ||
            items.distributorKey1 == null ||
            items.distributorPwd1 == null
        ) {
            // must contain author and at least 1 distributor certificate
            throw new Error('Failed to get ProfileItems');
        }

        try {
            let authorPwd = await cryptUtil.decryptPassword(items.authorPwd);
            this.profileInfo.author = cryptUtil.parseP12File(
                items.authorKey,
                authorPwd
            );

            let distributorPwd1 = await cryptUtil.decryptPassword(
                items.distributorPwd1
            );
            this.profileInfo.distributor1 = cryptUtil.parseP12File(
                items.distributorKey1,
                distributorPwd1
            );

            if (items.distributorKey2 && items.distributorPwd2) {
                let distributorPwd2 = await cryptUtil.decryptPassword(
                    items.distributorPwd1
                );
                this.profileInfo.distributor2 = cryptUtil.parseP12File(
                    items.distributorKey1,
                    distributorPwd2
                );
            }
    ...
    signPackage(projectRoot, userExcludeFiles) {
        ...
        fs.writeFileSync(authorSigFile, authorSignatureXml, {
            encoding: 'utf-8'
        });

        let distributorSig1 = new Signature(
            'DistributorSignature',
            projectRoot
        );
        distributorSig1.sign(
            this.profileInfo.distributor1.privateKey,
            userExcludeFiles
        );
        ...
        fs.writeFileSync(distributorSigFile1, distributorSignatureXml1, {
            encoding: 'utf-8'
        });

        let distributorSignatureXml2 = '';
        if (this.profileInfo.distributor2) {
            let distributorSig2 = new Signature(
                'DistributorSignature',
                projectRoot
            );
            distributorSig2.sign(
                this.profileInfo.distributor2.privateKey,
                userExcludeFiles
            );
            ...
            let distributorSigFile2 = path.resolve(
                projectRoot,
                `signature2.xml`
            );
            fs.writeFileSync(distributorSigFile2, distributorSignatureXml2, {
                encoding: 'utf-8'
            });
        }
    }
