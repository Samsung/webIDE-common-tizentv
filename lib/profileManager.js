...
module.exports = class ProfileManager {
    ...
    registerProfile(profileName, authorCA, authorCertPath, authorPassword, distributorCA, distributorCertPath, distributorPassword) {
        ...
        if (authorPassword != '') {
            authorPwdFile = path.resolve(path.dirname(authorCertPath), path.basename(authorCertPath, '.p12') + '.pwd');
            cryptUtil.encryptPassword(authorPassword, authorPwdFile);
        }
        if (distributorPassword != '') {
            distributorPwdFile = path.resolve(path.dirname(distributorCertPath), path.basename(distributorCertPath, '.p12') + '.pwd');
