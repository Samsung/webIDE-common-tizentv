...
module.exports = class TVWebApp {
  ...
  async launchOnEmulator(chromeExecPath, isDebug) {
    ...
    try {
      ...
      console.error(
        '[webide-common-tizentv]projectHelper.launchOnEmulator() error =' + err
      );
      throw err;
    }
  }

  async launchOnTV(tvIP, chromeExecPath, isDebug, portForward) {
    ...
    try {
      ...
      if (isDebug) {
        await launcher.debugApp(portForward);
        launcher.openChromeDevTool(chromeExecPath);
      } else {
        await launcher.executeApp();
      }
    } catch (err) {
      console.error(
        '[webide-common-tizentv]projectHelper.launchOnTV() error =' + err
      );
      throw err;
    }
  }
      ...
      jsonTemp.widget.feature.forEach(feature => {
        if (feature.$.name.startsWith('http://tizen.org/feature/screen.size')) {
          if (feature.$.name == 'http://tizen.org/feature/screen.size.all') {
            ...
            if (
              feature.$.name != 'http://tizen.org/feature/screen.size' &&
